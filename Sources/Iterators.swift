...
public extension RecurrenceRule {
    ...
    public func occurrences(between date: Date, and otherDate: Date, inclusive: Bool = false, endless endlessRecurrenceCount: Int = Iterator.endlessRecurrenceCount) -> [Date] {
        ...
        let ruleJSONString = toJSONString(endless: endlessRecurrenceCount)
        let _ = Iterator.rruleContext?.evaluateScript("var rule = new RRule({ \(ruleJSONString) })")
        guard let betweenOccurrences = Iterator.rruleContext?.evaluateScript("rule.between(new Date('\(beginDateJSON)'), new Date('\(untilDateJSON)'), \(inclusive.description))").toArray() as? [Date] else {
            return []
        }
